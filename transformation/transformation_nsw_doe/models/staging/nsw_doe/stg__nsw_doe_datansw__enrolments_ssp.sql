with source as (
    select *, from {{ source('raw', 'raw_nsw_doe_datansw_enrolments_ssp') }}
),

renamed as (
    select
        {{ adapter.quote("operational_directorate") }} as school_performance_directorate,
...
final as (
    select
        ...
        cast(trim(enrolments_ssp) as DECIMAL) as enrolments_ssp,
    from unpivoted
)
